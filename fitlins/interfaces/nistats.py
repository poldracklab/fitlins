...
def prepare_contrasts(contrasts, all_regressors):
    ...
    for contrast_info in contrasts:
        ...
        missing = len(contrast_info.conditions) != len(contrast_info.weights) or any(
            cond not in all_regressors for cond in contrast_info.conditions
        )
        if not missing:
                ...
                weights = np.array(
                    [
                        [
                            ...
                            if col in contrast_info.conditions
                            else 0
...
class FirstLevelModel(
    NistatsBaseInterface, FirstLevelEstimatorInterface, SimpleInterface
):
    ...
    def _run_interface(self, runtime):
        ...
        if isinstance(img, nb.dataobj_images.DataobjImage):
            ...
            if (
                max(
                    np.abs(slope32 - img.dataobj._slope),
                    np.abs(inter32 - img.dataobj._inter),
                )
                < 1e-7
            ):
        ...
        for name, weights, contrast_test in prepare_contrasts(
            ...
            if is_cifti:
                ...
                maps = {
                    map_type: dscalar_from_cifti(
                        img, getattr(contrast, map_type)(), map_type
                    )
                    for map_type in [
                        "z_score",
                        "stat",
...
class SecondLevelModel(
    NistatsBaseInterface, SecondLevelEstimatorInterface, SimpleInterface
):
    def _run_interface(self, runtime):
        import nibabel as nb
        from nilearn.glm import first_level as level1
            ...
            if model_type == "Meta":
                ...
                if is_cifti:
                    ...
                    maps = {
                        ...
                        "effect_variance": dscalar_from_cifti(
                            img, ffx_var, "effect_variance"
                        ),
